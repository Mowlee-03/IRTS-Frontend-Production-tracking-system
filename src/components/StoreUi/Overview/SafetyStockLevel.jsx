...
import { LinearProgress, Step, StepLabel, Stepper,  StepConnector,
...
const CustomStepIcon = ({ bg }) => (
  ...
  return (
    <div className="bg-white rounded-xl  shadow-md border border-gray-200 h-full flex flex-col gap-2 pb-3">
      <h2 className="text-md font-semibold text-gray-900 pl-3 pt-2">Safety Stock Level</h2>
      <div className="flex flex-1 px-3">
        <Stepper 
        sx={{py:'26px'}}
        orientation="vertical" connector={<CustomConnector />} nonLinear>
          {stockLevels.map((level, index) => (
            ...
            <div className="bg-g flex-1 flex flex-col justify-between">
                {stockLevels.map((level, index) => (
                <div 
                style={{
                    background:level.bgColor
                }}
                key={index} className={` px-3 shadow-sm border border-gray-200 rounded-xl `}>
                    ...
                    <div className="flex-1 py-3">
                        <div className="flex justify-between">
                            ...
                            className="text-lg font-medium ">{level.percentage}%</span>
                        </div>
                        <div className="">
                            <p className="text-xs xl-plus:text-sm text-gray-600">{level.subtitle}</p>
                            <LinearProgress
                            variant="determinate"
